...
// New Advent Calc
// Ref: https://www.codeproject.com/articles/10860/calculating-christian-holidays
function Advent($Y) {
  $weeks = 4;
  $correction = 0;
  $new_christmas = date($Y."-12-25");
  $christmas_day_int = date("N", strtotime($new_christmas));

    if ($christmas_day_int != 7)
    {
        $weeks--;
        $correction = ($christmas_day_int - 1);
    }
    return date("Y-m-d", strtotime(-1 * (($weeks * 7) + $correction)."days", strtotime($new_christmas)));
}
$this_advent = Advent($this_year);
...
function get_seasons($name, $startend, $year)
{
  ...
  // We don't check for date here, just use Year Provided as base year.
	...
	$fn_this_advent = Advent($fn_last_year);
...
// echo "<br/>Adevnt func test = " . Advent("2016");
// echo "<br/>$new_christmas = " . date($this_year."-12-25");
// echo "<br/>$christmas_day_int = " . date("N", strtotime($this_christmas));
// echo "<hr/>";
